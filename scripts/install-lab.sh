...
# Deploy the components individually for better control
...
helm install prometheus-pushgateway prometheus-community/prometheus-pushgateway \
  ...
  --values ../helm/values/prometheus-pushgateway.yaml
...
kubectl wait --for=condition=ready pod -l "app.kubernetes.io/name=opentelemetry-collector" --namespace monitoring --timeout=120s || echo "Warning: OpenTelemetry Collector pods not ready in time, continuing..."
kubectl wait --for=condition=ready pod -l "app=prometheus" --namespace monitoring --timeout=120s || echo "Warning: Prometheus pods not ready in time, continuing..."
...
# Update service names in configmap to match individual deployments
sed -i "s/prometheus-pushgateway.monitoring/prometheus-pushgateway-prometheus-pushgateway.monitoring/g" ../sample-app/k8s/configmap.yaml
sed -i "s/otel-collector.monitoring/otel-collector-opentelemetry-collector.monitoring/g" ../sample-app/k8s/configmap.yaml
sed -i "s/prometheus-server.monitoring/prometheus-server.monitoring/g" ../sample-app/k8s/configmap.yaml
...
echo "1. Access Grafana and open the Push Gateway Metrics dashboard"
