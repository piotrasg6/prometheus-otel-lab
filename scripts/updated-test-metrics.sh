...
echo ""
echo "3. Check that OpenTelemetry Collector is scraping Push Gateway:"
echo "   kubectl logs -n monitoring \$(kubectl get pods -n monitoring -l app.kubernetes.io/component=opentelemetry-collector -o jsonpath='{.items[0].metadata.name}') | grep pushgateway"
